    ...
    <main class="flex-grow flex items-center justify-center p-6">
        <div class="w-full max-w-md">
            <div class="text-center mb-8">
                ...
                <p class="text-slate-500 font-medium mt-2 px-6">We've sent a 6-digit security code to your email
                    address.</p>
            </div>
            ...
            <div class="glass-card p-10 rounded-[2.5rem] shadow-2xl shadow-slate-200/60">
                ...
                @if (session('error'))
                    <p class="bg-red-50 text-red-700 p-4 rounded-2xl mb-4 border border-red-100 text-sm font-bold">
                        {{ session('error') }}
                    </p>
                @endif
                ...
                <form method="POST" action="{{ route('auth.verify.email') }}">
                    ...
                    <div class="mb-8 text-center">
                        <label
                            class="block text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Verification
                            Code</label>
                ...
                <div class="text-center mt-6">
                    <p class="text-sm text-gray-500 mb-2 font-medium">Didn't receive the code?</p>
                    <form method="POST" action="{{ route('auth.resend.code') }}">
                        @csrf
                        <button type="submit" class="text-blue-600 hover:text-blue-700 font-bold transition-colors">
                            Resend Email
                        </button>
                    </form>
                </div>
